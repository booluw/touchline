...
## Part 8 — `internal/squad` reference implementation notes

> **Scope note:** this section is implementation guidance, not a new design proposal — it doesn't introduce anything Part 7 hasn't already approved. It exists to remove ambiguity before S04-02 engineering starts, by turning the Part 7 decisions into a concrete package sketch.

### Where this lives, and where it doesn't

`internal/squad` is orchestration-layer code — it reads real persisted data (`player.player_attributes`, `player.player_hidden_traits`, `player.player_emotional_states`) and computes the scalar modifiers that `pkg/matchsim` consumes. It is **not** part of `pkg/matchsim`, and nothing here changes that boundary: the engine stays pure, DB-free, and receives only the already-computed `Attack`/`Defense`/`PenaltyConversionRate` values on `Team`, exactly as Parts 2 and 6 describe. Part 7 §4 confirms this split is a requirement, not an implementation detail up for debate.

### Key-player selection (Part 7 §1)

```go
package squad // internal/squad
...
type SquadMember struct {
    PlayerID         uuid.UUID
    Position         string  // "GK", "CB", ..., used to anchor the priority tie-break
    IsCaptain        bool
    AttributeWeight  float64 // this player's share of the XI's aggregated Attack/Defense
    Hidden           PlayerHiddenTraitsSnapshot // consistency, temperament, pressure_handling, professionalism
    CurrentSentiment int     // signed; severity is checked via abs()
}

// SelectKeyPlayers implements PM Part 7 §1 exactly: the top 3–5
// position-weighted attribute contributors in the starting XI — with GK,
// Captain, Primary Striker, and Playmaker taking priority in ties — unioned
// with any starter whose |CurrentSentiment| >= SevereEmotionalThreshold,
// even if their attribute weight wouldn't otherwise qualify them.
func SelectKeyPlayers(xi []SquadMember, stakes FixtureContext) []PlayerPerformanceInput {
    ranked := rankByAttributeWeight(xi) // ties broken by position priority: GK > Captain > ST > AM
    selected := topN(ranked, 3, 5)      // 3–5 per PM's approved band

    for _, m := range xi {
        if abs(m.CurrentSentiment) >= SevereEmotionalThreshold && !contains(selected, m) {
            selected = append(selected, m) // severe emotional state always qualifies, regardless of rank
        }
    }
...
`rankByAttributeWeight`, `topN`, `contains`, and `toPerformanceInput` are private helpers local to the package — deliberately not exported, since the only supported entry point into this subsystem is `SelectKeyPlayers` followed by `ComputePlayerPerformanceFactor` (Part 6, Section 2.8) per player selected.

### Trait visibility (Part 7 §2)

Part 7 §2 makes visibility a hard requirement, not a UI team's discretion — so the labeling logic belongs in `internal/squad` itself (versioned, testable, and reused by both the scouting screen and the pre-match lineup selector), rather than being re-derived independently on the frontend:

```go
// TraitTagThresholds is tuning data, not a hardcoded cutoff — same pattern as
// pkg/matchsim's Tuning struct, kept in a versioned config block so labels
// can be recalibrated without a code change.
type TraitTagThresholds struct {
    ThrivesUnderPressureMin int // PressureHandling >= this → "Thrives Under Pressure"
    VolatileTemperamentMax  int // Temperament <= this → "Volatile Temperament"
    InconsistentMax         int // Consistency <= this → "Inconsistent"
    // ...additional tags as needed; each threshold independently tunable
}

// ScoutingTags turns a player's raw hidden traits into the PM-mandated
// descriptive labels (Part 7 §2), e.g. "Thrives Under Pressure",
// "Volatile Temperament". Feeds the scouting-report screen.
func ScoutingTags(h PlayerHiddenTraitsSnapshot, thresholds TraitTagThresholds) []string

// LineupWarning is generated pre-match for any key player whose computed
// PlayerPerformanceFactor is materially negative (e.g. below some tunable
// floor, say 0.95), with a full Explanation breakdown attached — this is
// what satisfies Part 7 §2's "Distracted by broken contract promise (-10%)"
// example, and PRD Principle 3 (explain consequences — never make the
// manager wonder why something happened).
func LineupWarning(p PlayerPerformanceInput, factor PlayerPerformanceFactor) (*Explanation, bool)
```

`LineupWarning`'s `Explanation` follows the same `{Subject, Score, Factors}` shape used everywhere else in the codebase (board confidence, job security, transfer desire) — for a lineup warning, `Factors` breaks down exactly which trait/sentiment inputs pushed the factor below neutral, so the manager sees the same kind of causal breakdown they'd get from a board-confidence explanation, not a bare number.

### What calls this, and when

`internal/match`'s pre-match aggregation step (already responsible for building `Team.Attack`/`Team.Defense` per Part 2, Section 2.1) is the caller:

1. Build the starting XI's `[]SquadMember` from persisted attributes/traits/emotional states.
2. `SelectKeyPlayers(xi, stakes)` → the subset worth varying individually.
3. For each selected player, `ComputePlayerPerformanceFactor` (Part 6) → scales that player's contribution before it's summed into `Team.Attack`/`Team.Defense`.
4. Separately (not blocking match aggregation), `ScoutingTags` feeds the scouting-report read path, and `LineupWarning` runs against the confirmed lineup before kickoff, surfacing through the squad-selection screen.

None of steps 2–4 touch `pkg/matchsim`; by the time `Simulate` is called, all of this has already been folded into two plain integers per team.

### Open items

None — Part 7 already resolved the design questions this section depended on (threshold, visibility requirement, bounded ranges). This section is implementation guidance only and doesn't require further PM sign-off unless engineering discovers a gap once real generated-attribute data exists to build `AttributeWeight` against.
