...
func (s *server) router() *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		admin := api.Group("/admin", s.requireAuth, s.requireAdmin)
		{
			...
			admin.POST("/cups/preview", s.handlePreviewCup)
			admin.PATCH("/cups/:id/qualification", s.handleSetCupQualification)
			admin.POST("/worlds/:id/countries/:countryID/cups/:cupID/campaign", s.handleStartCupCampaign)
			admin.POST("/worlds/:id/cups/:cupID/campaign", s.handleStartRegionalCupCampaign)
