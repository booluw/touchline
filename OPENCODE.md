...
frontend/
  ...
  stores/{club,squad,finance,transfers,social,realtime,match}.ts   — Pinia; realtime owns the single socket lifecycle + latest world_tick/error (S02-04); match (S04-03) loads the persisted feed over REST then live-appends deduped `match_tick` events
  server/index.ts  — BFF health/status only (no game logic)
  pages/{index, auth/login, competitions, matches/[fixtureId]}.vue — competitions table links to each live match screen; matches/[fixtureId] renders the S04-03 feed (server-computed score/header + ordered event list)
...
6. **~~Deterministic match engine + live matchday runner~~** ✅ Done (S04-02, OPD-21): the pure `pkg/matchsim` simulation (seed + teams + ordered inputs ⇒ identical feed) is integrated by the worker: the daily branch kicks off due fixtures and launches a per-world live runner (`internal/matchday`) that paces each match in real time at the configured cadence, finalizes deterministically at full time, applies results to the standings, and rehydrates after crashes; at-launch sweep picks up any world already mid-match. Managers can issue live substitution/tactic changes into the ordered input stream. Remaining: the HTTP live-control surface, and S05-01 tactical modulation (engine consumes tactic_change inputs with no numeric effect).
