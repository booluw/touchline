...
6. **~~Deterministic match engine + live matchday runner~~** ✅ Done (S04-02, OPD-21): the pure `pkg/matchsim` simulation (seed + teams + ordered inputs ⇒ identical feed) is integrated by the worker: the daily branch kicks off due fixtures and launches a per-world live runner (`internal/matchday`) that paces each match in real time at the configured cadence, finalizes deterministically at full time, applies results to the standings, and rehydrates after crashes; at-launch sweep picks up any world already mid-match. Managers can issue live substitution/tactic changes into the ordered input stream. Remaining: the S04-03 observer feed over the realtime socket, the HTTP live-control surface, and S05-01 tactical modulation (engine consumes tactic_change inputs with no numeric effect).
