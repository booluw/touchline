...
| **Fixture pacing** | Matchday `k` of a season lands on game-day `floor((k−1) × days_per_week / matchdays_per_week) + 1` from the season anchor: defaults give days 1,3,5,8,10,12,… All knobs are per-league `competition_rules.scheduling_rules` JSONB — `matchdays_per_week`, `days_per_week` (falls back to the world `calendar.days_per_week`), `kickoff_hours` (UTC rotation, default `[15,18,20]`). Read at season creation; re-read when serving the calendar. | `internal/competition/pacing.go`, `calendar.go`. |
