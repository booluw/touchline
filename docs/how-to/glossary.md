...
| **Cup qualification** | `competition.cup_qualification` rows describing who a cup draws from (`cup_id`, `league_id`, `from_position`, `to_position`), unique per (cup, league). Consumed by the qualification engine (IM07) and the resolution sweep (IM09); dormant after this milestone. | migration 0052 (IM06). |
