...
| **Season** | Per league, one at a time. Statuses: `in_progress` (season 1), `upcoming` (rollover season 2+; already fixture-scheduled and kicks off on its own), `completed`. `season_number = MAX + 1`. Label from the boot reference date. | See [seasons.md](seasons.md). |
| **StartSeason** | Service seam creating season #1 (entries + deterministic double round-robin, one matchday per game-day, fixtures ≥2 game-days apart). Second call → `ErrLeagueAlreadySeeded`. |
| **Rollover** | When every league in a country applies its final result: `SEASON_COMPLETED` → `CLUB_PROMOTED`/`CLUB_RELEGATED` → `SEASON_CREATED`, all in one atomic transaction; next season's fixtures anchor the day after the old last matchday. |
