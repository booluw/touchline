...
| **Membership** | A club's `competition.club_competitions` row; `role='league'` = domestic league seat. **Offers require it**; a club plays in as many leagues as declared. A `role='league'` row is written **once per club for life** (`uq_club_one_league`) — a promoted club keeps its now-stale row, so *current* placement is derived from the latest non-completed season entries, never the membership alone. |
| **League-less club** | A club holding **no** `role='league'` membership anywhere. Solely these clubs are admission candidates (`POST /api/admin/worlds/:id/countries/:countryID/clubs/:clubID/league`, IM14) and the rollover's auto-fill pool. Once a club holds a row it is never "league-less" again, even after promotion. |
| **Auto-fill** | The rollover backstop (IM14) that tops a league's next season up to `team_count`: first the country's league-less clubs (matched by `lower(country)`), then freshly generated AI clubs — so the fixture schedule is never holey. Declared members sort ahead of it. | `internal/competition/membership.go`. |
...
| **StartSeason** | Admin endpoint `POST /api/admin/worlds/:id/leagues/:leagueID/season` creating season #1 (entries + deterministic paced double round-robin: `matchdays_per_week` default 3 over `days_per_week` default 7). Second call → `ErrLeagueAlreadySeeded`. Endpoint maps `404`/`409` for world/league/state errors. | `internal/competition/seeding.go`, `competition_handlers.go`. |
| **Fixture pacing** | Matchday `k` of a season lands on game-day `floor((k−1) × days_per_week / matchdays_per_week) + 1` from the season anchor: defaults give days 1,3,5,8,10,12,… All knobs are per-league `competition_rules.scheduling_rules` JSONB — `matchdays_per_week`, `days_per_week` (falls back to the world `calendar.days_per_week`), `kickoff_hours` (UTC rotation, default `[15,18,20]`). Read at season creation; re-read when serving the calendar. **Superseded by `allowed_weekdays` when a weekday set resolves (IM05).** | `internal/competition/pacing.go`, `calendar.go`. |
| **Allowed weekdays** | ISO `1`..`7` array (Mon=1 … Sun=7) under `scheduling_rules->'allowed_weekdays'` (per league/cup) or `world.countries.default_scheduling_rules->'allowed_weekdays'` (country default). Resolution: per competition → country → legacy day formula. When a set resolves, matchday 1 = earliest allowed weekday ≥1 day after the anchor and every later matchday = earliest allowed weekday ≥**2 game-days** later (the two-day rest floor is structural). `PATCH /admin/leagues/:id/scheduling` (re-paces live leagues: frozen matchdays stay, unstarted ones slide forward), `/admin/cups/:id/scheduling` (persists for future campaigns), `/admin/worlds/:id/countries/:countryID/scheduling` (propagates to fallback leagues). Clearing never moves fixtures. | `internal/competition/weekdays.go`, `scheduling.go`. |
| **Season calendar** | `GET /api/competitions/:id/calendar`: the active season grouped by game-week (`week = game_day ÷ days_per_week`), each matchday holding its fixtures with its kickoff `scheduled_at`. | `internal/competition/calendar.go`. |
| **Club fixtures** | `GET /api/clubs/:id/fixtures`: one club's fixtures ordered by kickoff, world-scoped to the caller. | `internal/competition/calendar.go`. |
| **Cup calendar anchoring** | The cup ladder's dates are planned **backward from the final** (IM05): the final = first allowed weekday ≥3 game-days after the country's latest league fixture; each earlier round sits `cupGap` game-days back (seeded, 3-biased near the final) snapped to a day no country league plays, honoring the cup's `allowed_weekdays` when declared. Round days are stamped on the ladder (`roundPlan.date`) at campaign start and honored by `materializeRound`. | `internal/competition/cup.go` (`planCupCalendar`, `findCupRoundDay`). |
| **Final date policy** | The cup's IM10 declaration on `competition.competitions`: `final_date_mode` (`calculated` default / `fixed`), `final_date` (the absolute date a fixed cup pins), `final_offset_days` (default 3 — the `K` in "K game-days after the scope's latest league fixture" for calculated cups). `calculated` re-derives the final each season deterministically; `fixed` pins one date and never recalculates. An unconfigured cup reproduces the IM05 anchor exactly. | `internal/competition/cup.go` (`resolveCupFinalDate`, `normalizeFinalDatePolicy`), migration 0054. |
| **Campaign final-date override** | A per-campaign edit (`PATCH /api/admin/cups/:id/final-date`) of a **calculated** cup's final: `final_date` re-stamps the final `roundPlan.date` on the persisted ladder (earlier unfrozen rounds walk backward), `null` clears it back to the computed anchor. Edits never move materialized rounds and are rejected once the final round has fixtures (`ErrCupFinalDateLocked`). | `internal/competition/cup.go` (`SetCupFinalDate`). |
| **Scheduling news** | A `world.news_stories` story of category `scheduling`, written transactionally with the fixtures it describes when a cup round materializes or a league re-pacing moves fixtures; `country_id` scopes it to one country's feeds (`NULL` = world-wide). Manager `GET /api/news` shows world-wide + own-country stories. | `internal/competition/scheduling.go`; `internal/admin/rename.go`. |
| **Domestic cup** | Admin-declared single-elimination knockout competition (IM04), `competition_type 'domestic_cup'`, `competition_rules.format 'knockout'`. All of a country's league clubs are eligible; the top N of tier 1 get a late bye and enter when X bottom-tier survivors remain. Cup campaign = one `competition.seasons` row; results advance `qualified`/`eliminated`→`champion` and never write league standings or trigger the rollover. | See [cup-competitions.md](cup-competitions.md); `internal/competition/cup.go`. |
| **Qualification rules** | The cup's staging variables, `competition_rules.qualification_rules` JSONB: `first_tier_bye` (N) and `survivor_threshold` (X) | `internal/competition/cup.go`. |
| **Golden goal** | The resolver for drawn cup ties: after regulation 90', the sim continues deterministically minute-by-minute and the first goal wins (minute >90, seed-stable). No penalties, no dedicated ET periods. | `pkg/matchsim` (IM04). |
| **Cup campaign** | Materialised cup season: `role='cup'` memberships (≤3 per club, `ErrCupLimit`), entries, and a lazily-built bracket — Round 1 drawn at start, later rounds materialised as winners advance, all from `world_seed ⊕ cup_id ⊕ round`. | `internal/competition/cup.go`. |
| **Late entry** | The cup's top-N bye clubs (`first_tier_bye`) that skip the qualifier rounds and enter the draw once exactly X bottom-tier survivors remain — the join round is materialised with a field of X+N. | See [cup-competitions.md](cup-competitions.md); `internal/competition/cup.go`. |
| **Region** | Admin-declared, world-scoped grouping of countries (`world.regions`, IM06). Each assigned country belongs to at most one region (`world.countries.region_id`, nullable); only the geography an admin actually assigns participates in regional cups (IM08). Deleting a region leaves its countries unassigned — countries are never deleted. | `internal/competition/regions.go`, migration 0052. |
| **League reputation** | The admin-managed `competition.competitions.reputation` 0–100 (IM06, reused from the previously dormant column). It drives the default continental-cup qualification band (IM08); the match engine never reads it. | `internal/competition/regions.go`. |
| **Position band** | The universal qualification primitive (IM06): a `competition.cup_qualification` row maps a cup to a league with `from_position..to_position`; a NULL `to_position` means "through last place". The league's last **completed** season provides the rank order. | migration 0052; `internal/competition/regions.go`. |
| **Reputation default band** | The IM08 wizard suggestion for a league's band: `1..4` at rep ≥90, `1..3` at ≥75, `1..2` at ≥60, else `1..1` (rep clamped 0–100). A default only — the admin edits every band and the engine reads only the committed rows. | `internal/competition/regional_cup.go` (`DefaultBandForReputation`). |
| **Regional cup** | An admin-declared `continental`-type knockout cup scoped to a **region**, `country_id NULL`, with a **soft tier** label and per-league **position bands**. The field is projected by the IM07 engine (bands over each league's last completed season + reigning-champion +1) at preview and campaign time; the calendar anchors to the **union of participating countries' league days**. Contested by `regional rules` (entry `regional_league_bands`). | IM08; `internal/competition/regional_cup.go`. |
| **Soft tier** | The regional cup's tier label (`competition.competitions.tier`): the wizard's default-band generator + IM09's cross-cup precedence comparison. It **never filters** at qualification time and is not a scheduling constraint. | IM08; `internal/competition/regional_cup.go`. |
| **Cup qualification (field)** | The projection a cup actually draws: the union of its position bands over each banded league's last **completed** season, plus the reigning-champion +1, computed deterministically by the qualification engine (IM07, `ComputeField`). A banded league with no completed season is surfaced as **unavailable** (list, never an error); the preview warns and campaign start refuses (`ErrQualificationUnavailable`). Overlaps against other continental cups are flagged per-club as **conflicts** (IM09 resolves them). | `internal/competition/qualify.go` (IM07), `regional_cup.go`. |
| **Reigning champion** | The club that won the cup's most recent completed campaign (`competition_entries.status='champion'`, newest completed season), resolved to its **current** league membership (`club_competitions role='league'`). Guaranteed a field seat: direct when its league has no band row or is unavailable, in-band as the +1 next-best, otherwise out-of-band on top of the field. | `internal/competition/qualify.go` (`ReigningChampion`). |
| **Next-best** | The +1 seat a champion finishes **inside** its league's band: the highest-ranked club past the band cut that isn't already entered (band+1 in field size). A champion below its band enters **out-of-band**; a band running to last place has no next-best. | `internal/competition/qualify.go`. |
| **Double qualification** | A club holding a seat in **more than one** continental cup at once — always the reigning champion, effectively (it holds a `champion_*` seat in the cup it won and also finished inside another cup's band). Resolved by the IM09 sweep so it ends up in exactly one. | See [cup-competitions.md](cup-competitions.md) §7; `internal/competition/resolve.go`. |
| **Tier precedence** | The IM09 sweep's strictest tie-break: a club claimed by two continental cups plays the one with the **higher tier integer** (tier 3 beats tier 1); no manager choice can override it. The forfeited cup's freed slot cascades to next-best. | `internal/competition/resolve.go`. |
| **Cup choice** | The manager's recorded opt-in for one of the club's projected continental cups (`competition.manager_cup_choices`, `UNIQUE(club_id, cup_id)`). Decides equal-tier conflicts before the defend/create tie-breaks; a choice for a cup the club isn't projected for is rejected `422`. Consumers are the IM09 sweep at campaign start and the cup-qualifications outlook. | `internal/competition/cup_choices.go`. |
| **Cascade replacement** | A club promoted into a cup by the resolution sweep when a double-qualified club forfeits that cup (`origin 'cascade_replacement'`). Like a champion it is **cap-exempt** (never denied by the 3-cup cap). News names the mover, both cups, and this replacement (or that the freed place stays open). | `internal/competition/resolve.go`, `cup_resolution.go`. |
...
| **Rollover** | When every league in a country applies its final result: `SEASON_COMPLETED` → `CLUB_PROMOTED`/`CLUB_RELEGATED` → `SEASON_CREATED`, all in one atomic transaction; the next season is created `upcoming`, anchored `lastFixtureDay + off-season gap`, and the daily tick flips it `in_progress` with a `SEASON_STARTED` event once its first fixture is due. Each league's next table = stayers + movers, then **declared members** (IM14), then **auto-fill**. | `internal/competition/rollover.go`, `season.go`, `membership.go`. |
| **Capacity change** | `PATCH /api/admin/leagues/:id/capacity` (IM14): raises `team_count` and/or restates `promotions`/`relegations`, all binding at the next season. `team_count` only rises (`ErrLeagueShrink`); the neighbour's reciprocal count is auto-adjusted in the same transaction so `ErrAdjacencyMismatch` stays impossible. | `internal/competition/membership.go`. |
